...
void countHitsEachModuleUD() {
    ...
    Char_t buffer[256];
    TString mapname = "../rc/channel_new.dat";
    ...
    for(Int_t ientry = 0; ientry < nentries; ientry++) {
        ...
        for(Int_t index = 0; index < nindex; index++) {
            ...
            if(pe[index] > PE_threshold) {
                if(ch_mod[index] >= 0 && ch_mod[index] <32){
                    countOverThreshold[module[index]]++;
                }
                else if(ch_mod[index] >= 32){
                    countOverThreshold[module[index]+4]++;
                }
            }
        }
    ...
    for (int i = 0; i < nHistograms; i++) {
        ...
        histograms[i]->Draw();
        if(i<4){
            c[i]->SaveAs(Form("../output/hit_count_module%dU.png", i));
        }
        else{
            c[i]->SaveAs(Form("../output/hit_count_module%dD.png", i-4));
        }
    }
}
