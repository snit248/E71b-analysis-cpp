  ...
  for(Int_t ientry=0; ientry<mergetree->GetEntries(); ientry++){
    ...
    //(Int_t iline=0; iline<nline; iline++){
      ...
      //f(ientry==140000){
        ...
        // 1つ目のビュー (Sideview) に表示
        c->cd(1);
        TPaveText *pt1 = new TPaveText(0.05, 0.01, 0.95, 0.05, "NDC"); // NDC (Normalized Device Coordinates) で座標を指定
        pt1->SetFillColor(0);
        pt1->SetTextAlign(12); // 左下に揃える
        pt1->AddText(Form("Unixtime: %d", unixtime[0])); // unixtime[0]を表示
        pt1->Draw();
